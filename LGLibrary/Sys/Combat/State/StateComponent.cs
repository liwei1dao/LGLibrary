...
{
    protected Dictionary<string, List<EntityState>> m_States = new();
    protected LinkedList<EntityState> m_FsmStateBases = new();
    public override void LGInit(Entity entity, params object[] agrs)
    {
        base.LGInit(entity);
    }
    ...
    /// </summary>
    /// <returns></returns>
    public EntityState GetCurrentFsmState()
    {
    ...
    /// <returns></returns>
    private bool CheckIsFirstState(EntityState aFsmStateBase)
    {
    ...
    {
        EntityState temp = GetState(stateName);
        if (temp == null)
            return;
    ...
    /// </summary>
    public bool ChangeState(EntityState aFsmStateBase)
    {
        EntityState tempFsmStateBase = this.GetState(aFsmStateBase.StateName);
    ...
    public bool ChangeState<T>(string stateName, int priority) where T : EntityState, new()
    {
        EntityState aFsmStateBase = this.GetState(stateName);
    ...
    private void InsertState(EntityState fsmStateToInsert, bool containsItSelf = false)
    {
        ...
        LinkedListNode<EntityState> current = this.m_FsmStateBases.First;
        while (current != null)
        {
        ...
        EntityState tempFirstState = this.GetCurrentFsmState();
        ...
        else //如果不包含自身，且current不为空，即代表非尾节点有自己的位置，就插入，否则代表所有结点优先级都大于自身，就直接插入链表最后面
        {
            ...
            {
                this.m_States.Add(fsmStateToInsert.StateName, new List<EntityState>() { fsmStateToInsert });
            }
        }
