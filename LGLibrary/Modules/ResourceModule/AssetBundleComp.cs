...
public class AssetBundleComp : ModelCompBase<ResourceModule>
{
    ...
    public override void LGLoad(ModuleBase module, params object[] agrs)
    {
        ...
        bool IsSucc = FilesExtend.IsKeepFileOrDirectory(AppConfig.AppAssetBundleAddress + "/VersionInfo.json");
        if (IsSucc)
        {

            string VersionStr = FilesExtend.ReadFileToStr(AppConfig.AppAssetBundleAddress + "/VersionInfo.json");
            ...
            {
                ModuleManager.Instance.StartCoroutine(AssemblyAsset(() =>
                {
                    string AssetStr = FilesExtend.ReadFileToStr(AppConfig.AppAssetBundleAddress + "/AssetInfo.json");
        ...
        else
        {
            ModuleManager.Instance.StartCoroutine(AssemblyAsset(() =>
            {
                string jsonStr = FilesExtend.ReadFileToStr(AppConfig.AppAssetBundleAddress + "/AssetInfo.json");
      ...
      string resPath = "file://" + AppConfig.GetstreamingAssetsPath + "/Res.zip";
        ...
        else
        {
            ...
            yield return ZipTools.UnzipFile(dowle.data, AppConfig.AppAssetBundleAddress, AppConfig.ResZipPassword, (string describe, float progress) =>
            {
                ...
                Module.LoadViewComp?.UpdateProgress(progress, "正在解压资源:" + describe);
                Log.Debug(string.Format("正在解压资源 {0}:{1}", progress, describe));
            });
            // if (IsHaveLoadView)
    ...
    {
        bool IsSucc = FilesExtend.IsKeepFileOrDirectory(AppConfig.AppAssetBundleAddress + "/AssetInfo.json");
        if (IsSucc)
        {
            string jsonStr = FilesExtend.ReadFileToStr(AppConfig.AppAssetBundleAddress + "/AssetInfo.json");
    ...
    public void LoadModuleAssetInfo(string ModuleName)
    {
        ...
        bool IsSucc = FilesExtend.IsKeepFileOrDirectory(modulepath);
        if (IsSucc)
        {
            string jsonStr = FilesExtend.ReadFileToStr(modulepath);
