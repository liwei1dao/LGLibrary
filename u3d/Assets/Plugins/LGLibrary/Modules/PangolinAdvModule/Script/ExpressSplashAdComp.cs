    ...
    public class ExpressSplashAdComp : ModelCompBase<PangolinAdvModule>, ISplashAdListener, ISplashAdInteractionListener
    {
        ...
        private void ShowAdv_Android(string advId)
        {
            var adSlot = new AdSlot.Builder()
                ...
                .SetExpressViewAcceptedSize(UnityEngine.Screen.width, UnityEngine.Screen.height)
                .Build();
            this.Module.AdNative.LoadSplashAd(adSlot, this);
        }
#endif
        ...
        {
#if UNITY_ANDROID
            this.Module.GetSplashAdManager().Call("destorySplashView", this.Module.GetActivity());
#else
                this.splashAd.Dispose();
                this.splashAd = null;
#endif
...
#elif UNITY_ANDROID
            if (ad.GetInteractionType() == this.Module.INTERACTION_TYPE_DOWNLOAD)
            {
                Debug.Log("splash is download type ");
                ad.SetDownloadListener(Module);
            }
            if (ad != null && this.Module.GetSplashAdManager() != null && this.Module.GetActivity() != null)
            {
                this.Module.GetSplashAdManager().Call("showSplashAd", this.Module.GetActivity(), ad.getCurrentSplshAd());
            }
#endif
        }
        #endregion
        ...
        {
#if UNITY_ANDROID
            if (type != this.Module.INTERACTION_TYPE_DOWNLOAD)
            {
                DestorySplash();
            }
#endif
